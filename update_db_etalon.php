...
if(!isset($_GET["go"])){
    echo "<a href='?go'>[ Запустить процесс ]</a> - запуск процесса создания бекапа с эталона и последующего его развертывания на локальном сервере.<br>";
    exit();
}

echo "<h1>Делаем бэкап с эталона</h1>";

$tmpfile=tempnam(sys_get_temp_dir(),"mysqldump_");
echo "Файл: $tmpfile<br>\n";

# анализ параметров соединения
$host=false;
$port=3306;
$db=false;
$conn=explode(";",$etalon_config["db"]["dns"]);
foreach($conn as $c){
    $cc=explode("=",$c);
    if($cc[0]=="mysql:host") $host=$cc[1];
    if($cc[0]=="port") $port=$cc[1];
    if($cc[0]=="dbname") $db=$cc[1];
}
$cmd="mysqldump -P{$port} -h{$host} -u {$etalon_config["db"]["u"]} -p{$etalon_config["db"]["p"]} --add-drop-table --compress --extended-insert --no-create-db --quote-names -v --result-file=$tmpfile $db";
echo "$cmd<br>\n";
echo "<pre>Лог работы\n";
echo system("$cmd 2>&1");
echo "</pre>";
...
importDBFromFile($tmpfile,$local);
